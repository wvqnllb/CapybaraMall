<template>
  <el-dialog
    ...
    <el-form-item label="订单状态【0->待付款；1->已付款；2->已发货；3->已完成；4->已取消；5->售后中；6->售后完成】" prop="status">
      <el-input v-model="dataForm.status" placeholder="订单状态【0->待付款；1->已付款；2->已发货；3->已完成；4->已取消；5->售后中；6->售后完成】"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '订单状态【0->待付款；1->已付款；2->已发货；3->已完成；4->已取消；5->售后中；6->售后完成】不能为空', trigger: 'blur' }
          ],
          deliveryCompany: [
